...
## defining function for manually creating the adminrc files from user input

def adminrcManual():
    # open and create the file for accepting user input
    outFile = open("admin.rc", "a")

    # Each of the following line pairs (except API Version) accept user input and then write the input to the adminrc file
    osAUTH = input("What is the OS_AUTH_URL?\n")
    print("export OS_AUTH_URL=" + osAUTH, file=outFile)
    # Line will most likeyl stay hardcoded in due to version being unlikely to change
    ...
    print("\nadmin.rc file created")

    # Closing file when finished writing
    outFile.close()
